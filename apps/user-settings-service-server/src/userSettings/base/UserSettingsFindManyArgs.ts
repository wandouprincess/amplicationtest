/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserSettingsWhereInput } from "./UserSettingsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserSettingsOrderByInput } from "./UserSettingsOrderByInput";

@ArgsType()
class UserSettingsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserSettingsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserSettingsWhereInput, { nullable: true })
  @Type(() => UserSettingsWhereInput)
  where?: UserSettingsWhereInput;

  @ApiProperty({
    required: false,
    type: [UserSettingsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserSettingsOrderByInput], { nullable: true })
  @Type(() => UserSettingsOrderByInput)
  orderBy?: Array<UserSettingsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserSettingsFindManyArgs as UserSettingsFindManyArgs };
